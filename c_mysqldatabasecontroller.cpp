...
QList<QVariantList> c_MySqlDatabaseController::exe(QString query, QString destDatabase)
{
    QList<QVariantList> resultsList;

    QSqlDatabase db = databases[destDatabase];
    ...
    if (db.open())
    {
        ...
        while (q.next()) {
                resultsList.push_back(q.boundValues());
            }
